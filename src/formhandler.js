...
// --- Phone utilities (US 10-digit, allow + formatting, parens, dashes, spaces)
function cleanUsPhone(raw) {
  ...
  if (digits.length === 11 && digits.startsWith("1")) {
    digits = digits.slice(1);                        // trim leading country code
  }
  if (digits.length !== 10) { 
    return null;    
  }         // must be exactly 10
  // Optional: disallow area/exchange starting with 0/1 (classic NANP rule)
  const area = digits.slice(0, 3);
  const exch = digits.slice(3, 6);
  if (/[01]/.test(area[0]) || /[01]/.test(exch[0])) {
    return null;
  }
  return {
    digits,                                         // "7155715751"
    pretty: `(${area}) ${exch}-${digits.slice(6)}`  // "(715) 571-5751"
  };
}
...
if (contactForm) {
    contactForm.addEventListener('submit', (event) => {
        try {
            ...
            // new phone field for contact form.
            const rawPhone = document.getElementById("phone")?.value || "";
            const phoneObj = cleanUsPhone(rawPhone);
            ...
            // Validate phone (US 10-digit; accepts formats like (715) 571-5751 or 715-571-5751)
            if (!phoneObj) {
            ...
            console.log('Form submitted successfully:', { name, email, phone: phoneObj.pretty, message, photo});
